...
class PPO:
    ...
    def __init__(
        ...
        # Upper body curriculum parameters
        ...
        # Upper body curriculum components
        if upper_body_cur_cfg is not None:
            ...
            self.upper_body_cur.setdefault("curriculum_threshold", 0.5)  # Will be scaled down over training
        else:
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            # Upper body curriculum loss
            if self.upper_body_cur:
                # Get the policy mean actions for the original (non-augmented) batch
                action_mean_for_cur = mu_batch  # This is the original batch size actions
                action_std_for_cur = sigma_batch  # This is the original batch size actions std
                
                # Apply upper body joint curriculum
                if self.upper_body_cur["joint_indices"] is not None:
                    # Only penalize specified joint indices
                ...
                # Compute L2 penalty to encourage zero actions on upper body joints and their std to discourage exploration
                ...
                if self.upper_body_cur["use_curriculum"]:
                    # Scale down the coefficient over time (simple linear decay)
                    # You might want to pass current iteration or other curriculum signal
