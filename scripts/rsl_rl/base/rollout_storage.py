...
class RolloutStorage:
    ...
    def recurrent_mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        padded_privileged_obs_trajectories, _ = (
            split_and_pad_trajectories(self.privileged_observations, self.dones)
            if self.privileged_observations is not None
            else (padded_obs_trajectories, trajectory_masks)
        )
        padded_rnd_state_trajectories, _ = (
            split_and_pad_trajectories(self.rnd_state, self.dones)
            if self.rnd_state_shape is not None
            else (None, None)
        )
        ...
        for _ in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                rnd_state_batch = (
                    padded_rnd_state_trajectories[:, first_traj:last_traj]
