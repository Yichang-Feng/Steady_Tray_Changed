...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # Check if we're using adapter configuration early for resume path
    ...
    # Check training mode: adapter (RL), distillation, or standard
    is_adapter = is_adapter_config(agent_cfg)
    ...
    if is_adapter or is_distillation:
        ...
        # Use adapter-specific wrapper for multiple observation groups
        # - Adapter (RL): policy, encoder, critic/adapted_critic
        # - Distillation: policy, student_encoder, teacher_encoder, critic
        env = AdapterRslRlVecEnvWrapper(env, clip_actions=agent_cfg.clip_actions)
        
        # Create adapter runner (handles both RL and distillation modes)
        ...
        # create runner from rsl-rl (standard version)
