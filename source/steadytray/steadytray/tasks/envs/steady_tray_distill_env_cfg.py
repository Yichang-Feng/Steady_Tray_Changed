...
from .steady_tray_env_cfg import SteadyTrayEnvCfg, TrayObservationsCfg, TraySceneCfg, TrayTerminationsPlayCfg

@configclass
class DistillSceneCfg(TraySceneCfg):
    """Configuration for the scene in the steady object distillation environment."""

    tray_camera_transform: FrameTransformerCfg = FrameTransformerCfg(
...
@configclass
class DistillObservationsCfg(TrayObservationsCfg):
    """Configuration for observations in the steady tray distillation environment."""
    
    teacher_encoder: TrayObservationsCfg.EncoderCfg = TrayObservationsCfg.EncoderCfg()
    ...
    class StudentEncoderCfg(ObsGroup):
        ...
        # 相机视角下的托盘观测 (添加延迟与噪声)
        camera_tray_obs = ObsTerm(
            func=mdp.CombinedCameraObjectObservations,
            params={
                "camera_sensor_cfg": SceneEntityCfg("tray_camera_transform"),
                "object_asset_cfg": SceneEntityCfg("tray"),
                "target_frame_name": "tray",
                "scale_event_term_name": None,
                "include_pos": True,
                "include_quat": True,
                "pos_scale": 1.0,
                "quat_scale": 1.0,
...
class DistillCommandCfg:
    """Configuration for the velocity command in the steady tray distillation environment."""
...
@configclass
class SteadyTrayDistillEnvCfg(SteadyTrayEnvCfg):
    """Configuration for the steady tray distillation environment."""
    ...
    commands: DistillCommandCfg = DistillCommandCfg()


@configclass
class SteadyTrayDistillPlayEnvCfg(SteadyTrayDistillEnvCfg):
